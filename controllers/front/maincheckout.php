...
class SpellpaymentMaincheckoutModuleFrontController extends \ModuleFrontController
{
    ...
    private function makePaymentParams($configValues, $order)
    {
        ...
        $redirect_url = $this->context->link->getModuleLink(
            $this->module->name, 'checkoutcallback', ['order_id' => $order->id]
        );
        $failure_url = $this->context->link->getModuleLink(
            $this->module->name, 'checkoutcallback', ['order_id' => $order->id, 'restore_cart_id' => $cart->id]
        );
        $cancel_url = $this->context->link->getModuleLink(
            $this->module->name, 'checkoutcallback', ['order_id' => $order->id, 'restore_cart_id' => $cart->id, 'is_cancel' => true]
        );
        $callback_url = $this->context->link->getModuleLink(
            $this->module->name, 'checkoutcallback', ['order_id' => $order->id, 'is_module_callback' => true]
        );
    ...
    public function initContent()
    {
        ...
        $paymentRs = $spell->create_payment($paymentParams);
